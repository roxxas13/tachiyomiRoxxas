...
enum class MangaField(
    vararg val aliases: String,
    val fieldOnly: Boolean = false,
) {
    TITLE("title"),
    AUTHOR("author"),
    ARTIST("artist"),
    ...
    companion object {
        private val lookup =
            entries
                .flatMap { field -> field.aliases.map { it.lowercase() to field } }
                .toMap()
...
data object EmptyQueryNode : QueryNode

data class GeneralQueryNode(
    val value: String,
    val negated: Boolean,
) : QueryNode

data class FieldQueryNode(
    val field: MangaField,
    val value: String,
    val negated: Boolean,
) : QueryNode
